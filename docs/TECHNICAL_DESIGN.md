...
The single-function `Generate` path uses `text/template` for simple mapper functions with pointer-based signatures:
...
Pointer mismatches between source and destination fields are common in real-world mapping scenarios. The tool uses pointer-based function signatures and handles all field-level combinations automatically, prioritizing safety (no nil dereference panics) over brevity.

### Function-Level Signature

All generated mappers use pointer-based signatures with nil checks:

```go
func Map<Src>To<Dst>(src *<Src>) *<Dst> {
    if src == nil { return nil }
    dst := &<Dst>{}
    // field assignments...
    return dst
...
### Field-Level Conversion Matrix

The `FieldPair.Conversion()` method classifies field pairs into one of three cases:

| Source Field | Destination Field | Conversion | Generated Code |
|---|---|---|---|
...
When both source and destination are named structs, the pointer conversion interacts with the sub-mapper call. The generator handles four combinations (note that sub-mappers always use pointer-based signatures):

| Source Field | Destination Field | Generated Code |
|---|---|---|
| `Struct` | `Struct` | `MapSrcToDst(&src.F)` |
| `*Struct` | `Struct` | Nil check, deref, map, zero-value fallback |
| `Struct` | `*Struct` | Map with address of local var |
| `*Struct` | `*Struct` | Nil check, deref, map, take address, nil fallback |

The `Struct` to `Struct` case (address the source field):
...
The `*Struct` to `*Struct` case (nil-safe with pointer passing):
...
Slices add another dimension. The nil check is applied at the slice level (not the element level for value elements), and `make` pre-allocates the destination slice. Since sub-mappers use pointer-based signatures, slice elements are always passed as pointers:

```go
if src.Emails != nil {
    dst.Emails = make([]*EmailInfoDTO, len(src.Emails))
    for i, v := range src.Emails {
        // For value elements, address them for the pointer-based mapper
        result := MapEmailInfoToEmailInfoDTO(&v)
        if result != nil {
            dst.Emails[i] = result
        }
    }
...
When slice elements are already pointers, the same pattern applies with direct passing:

| Source Element | Destination Element | Loop Body |
|---|---|---|
| `T` | `T` | `result := MapFn(&v); if result != nil { dst[i] = *result }` |
| `*T` | `T` | `if v != nil { result := MapFn(v); if result != nil { dst[i] = *result } }` |
| `T` | `*T` | `dst[i] = MapFn(&v)` |
| `*T` | `*T` | `dst[i] = MapFn(v)` |
