...
// Deprecated: MatchCross is retained for backward compatibility during the
// matcher unification (Task 6 of qualified-refs plan). Callers should switch
// to Match. This wrapper is removed in Task 9.
//
// MatchCross performs cross-package field matching. The internal struct carries
// the mapper bind/bind_json tags. getters maps external field names to getter
// methods (may be nil).
func MatchCross(internal, external *loader.StructInfo, getters map[string]loader.GetterInfo) CrossMatchResult {
	// ToExternal: internal fields drive the lookup; bind tags live on internal.
	// We iterate internal fields and look up into external — this is src=internal,
	// dst=external, so bind tags on the *source* (internal) side must be honored.
	// The new Match iterates dst fields and reads bind tags from dst. To keep the
	// same semantic (internal bind tags drive matching), we pass internal as dst
	// and external as src for the purpose of tag reading, then swap the pair.
	//
	// Instead, replicate the original logic by building a synthetic dst (internal)
	// with bind tags intact and letting Match do the lookup into src (external).
	// Since bind tags are on internal and Match reads them from dst, we call
	// Match(external, internal, ...) for both directions and swap pairs as needed.

	// ToExternal: src=internal -> dst=external.
	// We want bind tags on internal (which will be dst in the Match call) to
	// select fields from external (src). Swap roles: Match(external, internal).
	toExtRaw := Match(external, internal, MatchOptions{})
	// Swap Src/Dst back so pairs read as internal→external.
	toExtResult := MatchResult{Unmatched: toExtRaw.Unmatched}
	for _, p := range toExtRaw.Pairs {
	...
	// FromExternal: src=external -> dst=internal, with getter probing on external (src).
	fromExtResult := Match(external, internal, MatchOptions{
		SrcGetters: getters,
